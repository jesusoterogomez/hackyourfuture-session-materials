...
// .wrangler/tmp/bundle-jDUKOr/checked-fetch.js
...
// src/index.js
import DOCS_HTML from "./e0024e18565f8616734fa5c8c2dbf07be8a42ef0-docs.html";
var CORS_HEADERS = {
...
__name(normalizePet, "normalizePet");
var src_default = {
  async fetch(request, env) {
    ...
    if (single && request.method === "GET") {
      const row = await env.paws_db.prepare("SELECT * FROM pets WHERE id = ?").bind(Number(single[1])).first();
    ...
    if (path === "/pets" && request.method === "POST") {
      ...
      const result = await env.paws_db.prepare(
        ...
        age,
        fee,
        available !== void 0 ? available ? 1 : 0 : 1,
        goodWithKids !== void 0 ? goodWithKids ? 1 : 0 : 1,
        image ?? null
      ).run();
      const newPet = await env.paws_db.prepare("SELECT * FROM pets WHERE id = ?").bind(result.meta.last_row_id).first();
...
// .wrangler/tmp/bundle-jDUKOr/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-jDUKOr/middleware-loader.entry.ts
